...
public class UpdateTaskStatusSessionTest {
	...
	public void setup() {
		...
		cli = new View();
		clock.setSystemTime(new DateTime(2015, 10, 12, 9, 0));
		...
		session = new UpdateTaskStatusSession(cli, ph, dth, uh.getDevelopers().get(0));
		...
		uh.getDevelopers().get(0).addReservation(ph.getProjects().get(0).getTasks().get(0), timeSpan);
		ph.getProjects().get(0).getTasks().get(0).addRequiredDeveloper(uh.getDevelopers().get(0));
	...
	public void useCase_SelectStatus_Cancel() {
		// ------------- Before running ---------------
		ArrayList<NormalTask> tasksOfProject1 = (ArrayList<NormalTask>) ph.getProjects().get(0).getTasks();
		ArrayList<NormalTask> tasksOfProject2 = (ArrayList<NormalTask>) ph.getProjects().get(1).getTasks();

		systemInMock.provideText("1\n0\n");
		session.run();

		// -------------- Check if task has been updated ---------
		assertEquals(branchOffice.getPh().getProjects().get(0).getTasks(), tasksOfProject1);
		assertEquals(branchOffice.getPh().getProjects().get(1).getTasks(), tasksOfProject2);
		Task updatedTask = ph.getProjects().get(0).getTasks().get(0);
	...
	public void useCaseTest_StatusExecution_Creation() {
		...
		// Enter cancel creation
		systemInMock.provideText("1\n1\n");
		session.run();
		assertTrue(log.getLog().contains("The selected task is now executing."));
	}
